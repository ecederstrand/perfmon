...
if MAKEOBJDIRPREFIX="$OBJDIR" make __MAKE_CONF="$MAKECONF" SRCCONF="$SRCCONF" \
   ...
   if tail -n 100 "$LOGDIR/buildkernel-$DATE.log" | grep "Signal 11"; then
      ...
      exit 1
         ...
         buildkernel > "$LOGDIR/buildkernel-noccache-$DATE.log" 2>&1; then
      log "Successfully built kernels GENERIC and $KERNEL (no ccache)"
   else
      log "buildkernel failed (no ccache)"
      log "Skipping this build. Waiting $SLEEP seconds.\nSee $LOGDIR/buildkernel-$DATE.log and $LOGDIR/buildkernel-noccache-$DATE.log for details.\n\n" 1
      skip=1
   fi  
fi
