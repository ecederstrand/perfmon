...
if MAKEOBJDIRPREFIX="$OBJDIR" make __MAKE_CONF="$MAKECONF" SRCCONF="$SRCCONF" \
   ...
   if tail -n 100 "$LOGDIR/buildworld-$DATE.log" | grep "Signal 11"; then
      ...
      exit 1
         ...
         > "$LOGDIR/buildworld-noccache-$DATE.log" 2>&1; then
      log "Successfully built world (no ccache)"
   else
      log "buildworld failed (no ccache)" 1
      log "Skipping this build. Waiting $SLEEP seconds.\nSee $LOGDIR/buildworld-$DATE.log and $LOGDIR/buildworld-noccache-$DATE.log for details.\n\n" 1
      skip=1
   fi  
fi
